...
const ChecklistForm: React.FC<Props> = ({ onSave, user, settings, checklists }) => {
  ...
  const linkedItemsCount = selectedEquipment
    ...
    : 0;
  const allAnswered = formData.items.length > 0 && formData.items.every(i => i.status !== null);

  const loadItemsForEquipment = (equipmentNo: string): ChecklistItem[] => {
    const equipment = settings.equipment.find(eq => eq.code === equipmentNo);
    if (!equipment) return [];

    return settings.items
      .filter(item => item.equipmentId === equipment.id)
      .filter(item => user.role !== 'OPERATOR' || item.area === user.area)
      .map(item => ({ ...item, status: null as Status }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasNC) {
      const managerPhone = backend.getManagerTargetPhone(managerId, settings);
      ...
      <form onSubmit={handleSubmit} className="p-10 bg-white dark:bg-slate-900">
        {step === 1 && (
          <div className="space-y-8 animate-fade-in">
                ...
                <div className="relative group">
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      setFormData({ ...formData, equipmentNo, items: loadItemsForEquipment(equipmentNo) });
                    }}
            ...
            <button
              type="button"
              disabled={!formData.equipmentNo || lockStatus.locked || isShiftTaken || linkedItemsCount === 0}
              onClick={() => setStep(2)}
            ...
            >
              {!formData.equipmentNo ? 'Selecione um Ativo' : linkedItemsCount === 0 ? 'Nenhum item vinculado ao ativo' : (lockStatus.locked || isShiftTaken) ? <><Lock size={18} /> Acesso Negado</> : <>Iniciar Inspeção <ArrowRight size={18} /></>}
            </button>
          </div>
        )}
