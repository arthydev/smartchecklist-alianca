...
const Dashboard: React.FC<Props> = ({ checklists, settings, user, onNavigate }) => {
  ...
  React.useEffect(() => {
    if (managerId) {
      backend.getUsers(managerId).then(users => {
  ...
  const inMaintenance = allEquipments.filter(e => backend.isEntityInAbsence(e.id, today, settings.absences));
  ...
  const missingChecklists = activeUsage.filter(eq =>
    !backend.isEntityInAbsence(eq.id, today, settings.absences) &&
    !todayChecklists.some(c => c.equipmentNo === eq.code)
  );

  // Ativos hoje (não estão de férias/licença)
  const activeOperatorsToday = operators.filter(u => !backend.isEntityInAbsence(u.id, today, settings.absences));
