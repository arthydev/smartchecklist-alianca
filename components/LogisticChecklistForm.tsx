...
const LogisticChecklistForm: React.FC<Props> = ({ onSave, user, settings }) => {
    ...
    React.useEffect(() => {
        const loadBrasiltecUsers = async () => {
            const users = await backend.getBrasiltecUsers(undefined, true);
            setBrasiltecUsers(users);
        };
        loadBrasiltecUsers();
    }, [user]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!isBrasiltecAbsent) {
            ...
            const isValid = await backend.validateBrasiltecUser(selectedBrasiltecUser, brasiltecPassword, managerId);
            if (!isValid) {
